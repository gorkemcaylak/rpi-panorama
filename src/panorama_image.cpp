...
vector<int> match_descriptors_a2b(const vector<Descriptor>& a, const vector<Descriptor>& b)
  ...
  for(int j=0;j<(int)a.size();j++)
    ...
    {
      dist = l1_distance(b[i].data, a[j].data);
      // cout<<"done first l1"<<endl;
      if(dist<best_distance){
        bind = i;
        best_distance = dist;

        // cout<<"bestdist: "<<best_distance<<" ind: "<<bind<<endl;
      }
    }
    ind.push_back(bind);
    // ind[j]=bind;
  ...
  // cout<<"expected: "<<a.size()<<" matches found: "<<ind.size()<<endl;
  return ind;
...
vector<Match> match_descriptors(const vector<Descriptor>& a, const vector<Descriptor>& b)
  ...
  vector<Match> m;
  vector<int> m1 = match_descriptors_a2b(a,b);
  vector<int> m2 = match_descriptors_a2b(b,a);
  ...
  for(int i=0; i<m1.size(); i++){
    ...
    int j = m1[i];
    if(j != -1 && i == m2[j]){
      // cout<<"a[i].data size: "<<a[i].data.size()<<" b[j].data size: "<<b[j].data.size()<<endl;
      float dist = l1_distance(b[j].data, a[i].data);
      // cout<<"past l1 2"<<endl;

      Match newM(&a[i], &b[j], dist); //pass the address?
      m.push_back(newM);
    }
    // }
  }
  // cout<<"best matches found: "<<m.size()<<endl;
...
Point project_point(const Matrix& H, const Point& p)
  ...
  int n = H.cols; //should be 3!
  Matrix H_(H);
  float factor = H_.data[n*n-1]; 
  // cout<<"f: "<<factor<<endl;
  // H_ = H_ / factor;                  //THIS???
  Matrix p_vec(n);
  Matrix pout_vec(n);
  float norm = sqrt(powf(p.x,2) + powf(p.y,2));
  p_vec.data[0] = p.x;///norm;
  p_vec.data[1] = p.y;///norm;
  // for(int i=2; i<n; i++){
  //     p_vec.data[i] = 1;
  // }
  p_vec.data[2] = 1;
  ...
  // float dummyx = (1+)  
  // NOT_IMPLEMENTED();
  // print_matrix(pout_vec);
  // pout_vec = pout_vec / pout_vec(0,2);
  // print_matrix(pout_vec);
  // pout_vec = pout_vec * norm;
  // print_matrix(pout_vec);

  return Point(pout_vec.data[0]/pout_vec.data[2],pout_vec.data[1]/pout_vec.data[2]);
  }
...
vector<Match> model_inliers(const Matrix& H, const vector<Match>& m, float thresh)
  ...
  for(auto each : m){
    Point projected = project_point(H, each.a->p);
    float dist = point_distance(projected, each.b->p);
    // cout<<"point: "<<each.a->p.x<<" "<<each.a->p.y<<" projected: "<<projected.x<<" "<<projected.y<<" expected: "<<each.b->p.x<<" "<<each.b->p.y<<" dist: "<<dist<<endl;
  ...
  // NOT_IMPLEMENTED();
  // cout<<"all: "<<m.size()<<" inliers: "<<inliers.size()<<endl;
  return inliers;
  }
...
Matrix RANSAC(vector<Match> m, float thresh, int k, int cutoff)
  ...
  Matrix Hprev;// = Matrix::translation_homography(256, 0);
  int many = 4; //4: 10/37 21/74   5:8/37 10/74   6:12/37 14/74    7:6/37 39/74
  vector<Match> manyvec;
  ...
  for(int i=0; i<k; i++){
    randomize_matches(m);
    manyvec.clear();
    for(int j=0; j<many; j++) manyvec.push_back(m[j]);
    // printf("%f %f \n",m[0].a->p.x,m[0].a->p.y);
    Hba = compute_homography_ba(manyvec);
    vector<Match> in = model_inliers(Hba, m, thresh);
    // cout<<"inliers: "<<in.size()<<endl;// > model_inliers(Hprev, m, thresh)
    // Hprev = Hba;
    if(in.size()>best){
      if(in.size()>3) Hbest = compute_homography_ba(in);
      else Hbest = Hba;
      best = in.size();
      // cout<<"best inlier: "<<best<<"/"<<m.size()<<endl;
      if(best>cutoff){
        return Hbest;
      }
    }
...
Image combine_images(const Image& a, const Image& b, const Matrix& Hba, float ablendcoeff)
  ...
  for(int j = 0; j < (int)h; ++j)
    for(int i = 0; i < (int)w; ++i)
      {
      // cout<<i<<" "<<j<<endl;
      Point proj_b = project_point(Hba, Point(i+dx,j+dy));
      if(proj_b.x < b.w && proj_b.x >=0 && proj_b.y < b.h && proj_b.y >=0 ){//< or <= ???
      // cout<<proj_b.x<<" "<<proj_b.y<<endl;
        for(int k = 0; k < a.c; ++k){
          float val = b.pixel_bilinear(proj_b.x,proj_b.y,k);

          if(c.is_nonempty_patch(i,j,k) && val != 0.0){// && c(i,j,k) != 0){
            c(i,j,k) =(1-ablendcoeff)* val + ablendcoeff*c(i,j,k);
          }
          else if(val == 0.0){
            c(i,j,k) = c(i,j,k);
          }
...
Image cylindrical_project(const Image& im, float f)
  ...
  cout<<"c size: "<<im.w/cos(hfov)<<" "<<im.h/cos(hfov)<<endl;
  int w_ = im.w/cos(hfov);
  int h_ = im.h/cos(hfov);
  // Matrix M_proj(3,4);
  // M_proj(1,0) = M_proj(2,0) = M_proj(0,1) = M_proj(2,1) = M_proj(0,3) = M_proj(1,3) = M_proj(2,3) = 0.0;
  // M_proj(0,0) = M_proj(1,1) = -1.0 * f;
  // M_proj(0,2) = im.w / 2.0;
  // M_proj(1,2) = im.h / 2.0; 
  // M_proj(2,2) = 1.0;
  ...
  for(int i=0; i<c.w; i++){
    for(int j=0; j<c.h; j++){
      ...
      int x_ = i;
      int y_ = j;
      float x = f * tan((x_-im.w/2)/f) +  im.w/2; //check!
      float y =  (y_-im.h/2) / cos((x_-im.w/2)/f) + im.h/2; //f*

      if(x < im.w && x >=0 && y <im.h&& y >=0 ){//< or <= ???

        for(int k=0; k<c.c; k++){
          c(x_,y_,k) = im.pixel_bilinear(x,y,k);
        }
      }
      else{
        for(int k=0; k<c.c; k++){
  ...
  // save_png(trim_image(c),"output/cylinderized");
...
Image spherical_project(const Image& im, float f)
  ...
  for(int i=0; i<c.w; i++){
    for(int j=0; j<c.h; j++){
      int x_ = i;
      int y_ = j;
      float theta = (x_-im.w/2)/f;
      float x = f * tan(theta) +  im.w/2; //check!
      // float phi = acos(f / sqrt(x_-im.w/2*x_-im.w/2 + y_-im.h/2*y_-im.h/2 + f*f));
      // float y =  tan(phi)/cos(theta) + im.h/2; //f*
      float y = f * tan((y_-im.h/2)/f)/cos(theta) + im.h/2; //f*
  ...
  // save_png(trim_image(c),"output/sphered");
