...
int main(int argc, char** argv)
  ...
  const string file1=argc==3?argv[1]:"pano/bar/12.jpg";
  const string file2=argc==3?argv[2]:"pano/bar/11.jpg";
  ...
  d1.input = bilinear_resize(d1.input,d1.input.w/10,d1.input.h/10);
  d2.input = bilinear_resize(d2.input,d2.input.w/3,d2.input.h/3);
  ...
  pangolin::Var<float> structure_blur("ui.structure_blur",3,0,7);
  pangolin::Var<int> nms_rad("ui.nms_rad",9,1,50,true);
  pangolin::Var<int> descriptor_window("ui.descriptor_window",15,5,20);
  pangolin::Var<bool> corner_method("ui.exact2ndEV",false,0,1);
  pangolin::Var<float> thresh_corner("ui.thresh_corner",0.05,1e-2,1e1,true);
  ...
  pangolin::Var<float> thresh_inliers_pix("ui.thresh_inliers_pix",7,0.1,100,true);
  pangolin::Var<int> iterations_ransac("ui.iters_ransac",500,1,30000,true);
