...
void do_columbia_peak(void)
  ...
  image_map im;
  load_images(im,indir,outdir,11,1,1310);  // im, dir, numpics (0..numpics-1),  PROJ_METHOD,  FOCAL_LEN
  //was spherical!!
  vector < thread > th; 
  
  th.push_back(thread([&](){create_panorama(im,"0-1","0","1" ,3,0,0.05,7,7,5,50000,100,0.5);}));
  th.push_back(thread([&](){create_panorama(im,"2-3","2","3" ,3,0,0.05,7,7,5,50000,100,0.5);}));
  th.push_back(thread([&](){create_panorama(im,"4-5","4","5" ,3,0,0.05,7,7,5,50000,100,0.5);}));
  th.push_back(thread([&](){create_panorama(im,"6-7","6","7" ,3,0,0.05,7,7,5,50000,100,0.5);}));
  th.push_back(thread([&](){create_panorama(im,"8-9","8","9" ,3,0,0.05,7,7,5,50000,100,0.6);}));
  for(auto&e1:th)e1.join();th.clear();
  
    save_images(im,outdir);

  th.push_back(thread([&](){create_panorama(im,"8--10",  "8-9",  "10"  ,3,0,0.15,11,7,5,50000,100,0.5);}));
  th.push_back(thread([&](){create_panorama(im,"0--3" ,  "0-1",  "2-3" ,3,0,0.04,11,7,5,50000,100,0.5);}));
  th.push_back(thread([&](){create_panorama(im,"4--7" ,  "4-5",  "6-7" ,3,0,0.04,11,7,5,50000,100,0.5);}));
  for(auto&e1:th)e1.join();th.clear();
  
    save_images(im,outdir);

  create_panorama(im,"4--10" , "4--7",  "8--10" ,2,0,0.04,10,7,5,50000,100,0.8);
  create_panorama(im,"all"   , "0--3",  "4--10" ,2,0,0.04,10,7,5,50000,100,0.8);
...
void do_rainier(void)
  ...
  create_panorama(im,"all" ,"0--3","4-5" ,2,0,0.05,7,7,5,50000,100,0.6);
...
void do_field(void)
  ...
  image_map im;
  load_images(im,indir,outdir,8,1,1300);  // im, dir, numpics (0..numpics-1),  PROJ_METHOD,  FOCAL_LEN
  
  vector < thread > th; 
  
  
  th.push_back(thread([&](){create_panorama(im,"2-3","2","3" ,2.5,0,0.03,8,6,5,50000,100,0);}));
  th.push_back(thread([&](){create_panorama(im,"4-5","4","5" ,3,0,0.05,11,7,5,50000,100,1);}));
  th.push_back(thread([&](){create_panorama(im,"6-7","6","7" ,2,0,0.03,9,6,4,50000,100,1);}));
  for(auto&e1:th)e1.join();th.clear();
  
  save_images(im,outdir);

  th.push_back(thread([&](){create_panorama(im,"4--7", "4-5",  "6-7" ,2,0,0.03,11,7,4,50000,100,1);}));
  for(auto&e1:th)e1.join();th.clear();
  
  create_panorama(im,"all"   , "4--7",  "2-3" ,2.5,0,0.05,9,7,4,50000,100,1);
...
void do_bar(void)
  {
  string indir="pano/bar/";
  string outdir="output/bar/";
  
  image_map im;
  load_images(im,indir,outdir,13,0,1000);  // im, dir, numpics (0..numpics-1),  PROJ_METHOD,  FOCAL_LEN
  vector < thread > th; 

  for(auto each : im.im){
    each.second = bilinear_resize(each.second,each.second.w/4,each.second.h/4);
  }
  
  th.push_back(thread([&](){create_panorama(im,"0-1","0","1" ,3,0,0.15,13,15,9,1000,100,0);}));    
  th.push_back(thread([&](){create_panorama(im,"2-3","2","3" ,3,0,0.15,15,17,11,1000,100,1);})); 
  for(auto&e1:th)e1.join();th.clear();
  save_images(im,outdir);

  th.push_back(thread([&](){create_panorama(im,"4-5","4","5" ,3,0,0.15,15,17,11,1000,100,0);})); 
  th.push_back(thread([&](){create_panorama(im,"6-7","6","7" ,3,0,0.15,15,17,11,1000,100,1);})); 
  for(auto&e1:th)e1.join();th.clear();
  save_images(im,outdir);

  for(auto each : im.im){
    each.second = bilinear_resize(each.second,each.second.w/6,each.second.h/6);
  }

 
  th.push_back(thread([&](){create_panorama(im,"4--7" , "4-5",  "6-7" ,2.8,0,0.15,17,13,9,1000,100,1);}));
  th.push_back(thread([&](){create_panorama(im,"0--3" , "0-1",  "2-3" ,2.8,0,0.15,17,13,9,1000,100,1);}));
  for(auto&e1:th)e1.join();th.clear();
  save_images(im,outdir);
  ...
  create_panorama(im,"all" , "0--3",  "4--7",2.8,0,0.15,17,13,9,1000,100,1);
